...
if __name__ == '__main__':
    # 加载模型
    model = YOLO(r'runs/train/yolov6/weights/best.pt')
    # 验证模型
    metrics=model.val(
        ...
        device='',  # 运行的设备，例如 cuda device=0 或 devi86.ce=0,1,2,3 或 device=cpu
        workers=8,  # 数据加载的工作线程数（每个DDP进程）
        save_json=False,  # 保存结果到JSON文件
        save_hybrid=False,  # 保存标签的混合版本（标签 + 额外的预测）
        conf=0.1,  # 检测的目标置信度阈值（默认为0.25用于预测，0.001用于验证）
        iou=0.6,  # 非极大值抑制 (NMS) 的交并比 (IoU) 阈值
        project='runs/val',  # 项目名称（可选）
        name='yolov6',  # 实验名称，结果保存在'project/name'目录下（可选）
