...
if __name__ == '__main__':
    # 加载模型
    model = YOLO(r'ultralytics/cfg/models/rt-detr/rtdetr-resnet50.yaml')  # 不使用预训练权重训练
    # model = YOLO(r'yolov8.yaml').load("yolov8n.pt")  # 使用预训练权重训练
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        data=r'D:\workspace\PycharmProjects\yolov8\ultralytics\cfg\datasets\mydata.yaml',
        ...
        name='rtdetr-resnet50',  # (str, optional) 实验名称，结果保存在'project/name'目录下
        ...
        single_cls=True,  # (bool) 将多类数据训练为单类
        ...
        label_smoothing=0.1,  # (float) 标签平滑（分数）
        nbs=64,  # (int) 名义批量大小
